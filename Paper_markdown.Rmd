...
Master2 <- inner_join(Master2, Alliance, by=c("ccodea", "ccodeb", "year"), copy=FALSE, suffix=c("x", "y"))
...
Master2 <- inner_join(Master2, USNaval, by=c("year"), copy=FALSE, suffix=c("x", "y"))
...
Master4 <- select(Master3, ccodea, ccodeb, year, Mid, Use_Force, Hostility, US_Naval_Cap, Same_Region, Contiguous, Joint_Dem, Relative_Cap, Major, Post_Cold_War, Lag_Mid)
summary(Master4)

Master5 <- full_join(Master4, Alliance, by=c("ccodea", "ccodeb", "year"), copy=FALSE, suffix=c("x", "y"))

Master5 <- Master5 %>%
    mutate(Ally =0)
Master5$Ally[Master5$neutrality==1] <- 1 
Master5$Ally[Master5$nonaggression==1] <- 1 
Master5$Ally[Master5$defense==1] <- 1 
Master5$Ally[Master5$entente==1] <- 1 
...
#Full Models
  #Logistic regression
MID_Full_Log <- glm(Mid ~ US_Naval_Cap + Relative_Cap + Joint_Dem + Major + Same_Region + Contiguous + Post_Cold_War + Lag_Mid, data = Master4, family=binomial)
dwplot(MID_Full_Log)
